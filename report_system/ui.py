...
def _render_create(application: ReportApplication) -> None:
    ...
    if st.button("Сформировать документ", type="primary"):
        try:
            with st.spinner("Извлечение фактов, генерация и проверка текста..."):
                document = application.confirm(application.extract(labels[selected], raw_input))
                _generate_and_render(application, document.id)
        except Exception as error:
            st.error(f"Ошибка формирования документа: {error}")
...
def _generate_and_render(application: ReportApplication, document_id: str) -> None:
    ...
    if not validation.valid:
        st.error("Не удалось получить текст без неподтверждённых утверждений. Текст не сохранён.")
        if validation.issues:
            st.dataframe([issue.model_dump(mode="json") for issue in validation.issues])
    elif document.generated_text and docx_path:
